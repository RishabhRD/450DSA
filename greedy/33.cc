...
bool can_rearrange(const string& str){
  unordered_map<char, int> count_map;
  for(auto c : str){
    count_map[c]++;
  }
  const int half_size = str.size() / 2;
  for(const auto& pr : count_map){
    if(pr.second > half_size) return false;
  }
  return true;
}
