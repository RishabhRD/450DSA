...
auto merge_overlapping_intervals(vector<pair<int, int>>& vec){
  sort(begin(vec), end(vec));
  vector<pair<int, int>> ans;
  ans.push(vec[0]);
  auto is_overlapping = [](const auto& a, const auto& b){
    return a.second >= b.first
  };
  for(int i = 1; i < vec.size(); i++){
    if(is_overlapping(vec.back(), vec[i])){
      ans.back() = {vec.back().first, vec[i].second};
    }else{
      ans.push_back(vec[i]);
    }
  }
