...
void dfs(int cur, vector<int> adj[], vector<bool>& visited, Func&& func){
  func(cur);
  visited[static_cast<size_t>(cur)] = true;
  for(const auto x : adj[cur]){
    if(not visited[static_cast<size_t>(x)]){
      dfs(x, adj, visited, func);
    }
  }
}
...
vector<int>dfsOfGraph(int V, vector<int> adj[]) {
  ...
  vector<int> dest;
  auto push_to_dest = [&](int i){
    dest.push_back(i);
  };
  dfs(0, adj, visited, push_to_dest);
  return dest;
}
